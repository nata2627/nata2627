...
**Причинный вывод на рыночных данных.** Событийный анализ: рыночная модель, аномальные и накопленные доходности. Разность разностей с контрольной группой по матчингу и формальной проверкой параллельных трендов. Рандомизационный вывод, плацебо-тесты, кластеризованные стандартные ошибки, поправки на множественность (Холм, Бенджамини—Хохберг).
...
**[moex-index-effect](https://github.com/nata2627/moex-index-effect)** — событийное исследование индексного эффекта на Московской бирже: что происходит с ценой и ликвидностью акции при включении в индекс IMOEX и исключении из него. 42 события за 2022–2026, рыночная модель и накопленные аномальные доходности, контрольная группа по матчингу, разность разностей по ликвидности, рандомизационный вывод на 1000 плацебо-дат, 15 проверок устойчивости, 39 тестов. Результат показан как есть, включая отрицательный: классического эффекта нет, зато после включения цена устойчиво снижается — CAR −18.3% за 60 дней, плацебо-p = 0.039.
`pandas` `statsmodels` `событийный анализ` `разность разностей` `матчинг`
...
**Эконометрика** МНК · диагностика предпосылок Гаусса—Маркова · логит/пробит · проверка гипотез · корреляционный анализ · эластичности · CAPM и расчёт беты · событийный анализ · разность разностей с фиксированными эффектами · матчинг · рандомизационный вывод · кластеризованные стандартные ошибки · поправки на множественность
